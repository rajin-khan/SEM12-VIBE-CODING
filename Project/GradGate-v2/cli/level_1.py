...
from display.formatter import print_banner, print_credit_tally
...
def main():
    ...
    parser.add_argument(
        "program_name", nargs="?", default=None, help="Program alias (CSE, BBA, EEE, etc.)"
    )
    parser.add_argument(
        "program_knowledge",
        nargs="?",
    ...
    if not Path(knowledge_path).exists():
        if args.program_name:
            print(
                f"Warning: Knowledge file '{knowledge_path}' not found. Running without program data."
            )
    ...
    if args.output:
        ...
        print_credit_tally(
            summary, args.transcript, program_info.full_name if program_info else args.program_name
        )
