...
def print_credit_tally(
    summary: CreditSummary, transcript_path: str, program_name: str | None = None
) -> None:
...
def print_semester_progression(
...
def print_grade_distribution(grade_distribution: dict[str, int], transcript_path: str = "") -> None:
    ...
    for tier_name, (_, grade_set) in GRADE_TIERS.items():
        tier_counts[tier_name] = sum(grade_distribution.get(g, 0) for g in grade_set)
...
def print_audit_report(result: AuditResult, full_report: bool = False) -> None:
    ...
    if result.waiver_bonus > 0:
        credits_str += (
            f" [dim](earned: {result.credits_earned:.1f} + waiver: {result.waiver_bonus:.0f})[/]"
        )
    header_table.add_row("Credits Completed", credits_str)

    cgpa_color = _cgpa_color(result.cgpa)
    header_table.add_row(
        "CGPA", f"[{cgpa_color}]{result.cgpa:.2f}[/] (min: {result.program.min_cgpa:.2f})"
    )
