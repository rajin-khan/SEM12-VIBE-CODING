...
def compute_cgpa(records: list[CourseRecord], waived: set[str] | None = None) -> float:
    ...
    for r in records:
        ...
        if existing is None:
            ...
            ex_pts_per = existing[1] / existing[0] if existing[0] > 0 else 0.0
...
def compute_cgpa_simple(
    records: list[CourseRecord], waived: set[str] | None = None
...
def compute_semester_progression(
    records: list[CourseRecord], waived: set[str] | None = None
...
def compute_major_cgpa(
    records: list[CourseRecord], major_courses: set[str], waived: set[str] | None = None
) -> float:
