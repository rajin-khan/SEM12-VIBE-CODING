...
from engine.prerequisites import PrereqViolation, check_prerequisites
from engine.program_loader import AlternativeGroup, MinorInfo, ProgramInfo
...
class DeficiencyReport:
    ...
    def total_missing(self) -> int:
        return (
            len(self.missing_ged)
            + len(self.missing_math)
            + len(self.missing_science)
            + len(self.missing_business)
            + len(self.missing_major)
            + len(self.missing_capstone)
            + len(self.missing_internship)
            + self.missing_trail
            + self.missing_concentration
            + self.missing_open_elective
        )
...
def _check_category(
    required_courses: list, passed: set[str], alternative_groups: list[AlternativeGroup]
) -> list[str]:
...
def _detect_minor(
    program_info: ProgramInfo, passed: set[str], explicit: str | None = None
) -> MinorInfo | None:
...
def _evaluate_minor(
    minor: MinorInfo, passed: set[str]
    ...
    if minor.elective_pick_count > 0 and not minor.elective_courses:
        ...
        any_extra = [c for c in passed if c.startswith(prefix) and c not in minor.required_courses]
        if not any_extra:
...
def run_audit(
    ...
    deficiencies.missing_math = _check_category(program_info.core_math, passed, [])
    deficiencies.missing_science = _check_category(program_info.core_science, passed, [])
    deficiencies.missing_business = _check_category(program_info.core_business, passed, [])
    deficiencies.missing_major = _check_category(program_info.major_core, passed, [])
    deficiencies.missing_capstone = _check_category(program_info.capstone, passed, [])
    deficiencies.missing_internship = _check_category(program_info.internship, passed, [])
    ...
    if detected_minor:
        ...
        minor_taken, minor_missing, minor_prereqs_met, minor_prereqs_missing = _evaluate_minor(
            detected_minor, passed
        )
    ...
    roadmap = _build_roadmap(
        ...
        credits_required,
        cgpa,
        program_info.min_cgpa,
        major_cgpa,
        conc_cgpa,
        conc_name,
        program_info.concentration_min_cgpa,
        deficiencies,
        failed_courses,
    )
    ...
    if detected_minor and not minor_completed:
        if minor_missing:
            roadmap.append(f"Complete minor ({minor_name}): {', '.join(minor_missing)}")
        if minor_prereqs_missing:
            roadmap.append(f"Complete minor prerequisites: {', '.join(minor_prereqs_missing)}")
...
def _build_roadmap(
    ...
    if credits_completed < credits_required:
        gap = credits_required - credits_completed
        steps.append(
            f"Earn {gap:.0f} more credits to reach the {credits_required}-credit requirement"
        )
