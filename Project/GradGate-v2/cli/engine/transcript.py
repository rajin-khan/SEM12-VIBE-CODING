    ...
    try:
        with open(path, encoding="utf-8-sig") as f:
            ...
            if reader.fieldnames:
                ...
                if missing_cols:
                    print(
                        f"Error: Transcript CSV is missing required column(s): "
...
def resolve_retakes(
    records: list[CourseRecord], equivalences: dict[str, set[str]] | None = None
) -> list[CourseRecord]:
    ...
    for canonical, group in groups.items():
        ...
        if len(gpa_records) == 1:
            ...
            if first_grade_ineligible:
                ...
                best = max(
                    gpa_records,
                    key=lambda x: (GRADE_ORDER.get(x.grade.upper(), -99), x.semester_sort_key),
                )
                for r in gpa_records:
                    if r is best:
                        if r.grade.upper() == "T":
...
def get_best_records(records: list[CourseRecord]) -> dict[str, CourseRecord]:
    ...
    for r in records:
        if r.status in ("Counted", "Waived", "Failed", "Transfer"):
            ...
            if existing is None or GRADE_ORDER.get(r.grade.upper(), -99) > GRADE_ORDER.get(
                existing.grade.upper(), -99
            ):
