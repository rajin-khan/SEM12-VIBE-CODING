...
from api.services.ocr import extract_transcript_csv
    ...
    (
        ...
        [
            ...
            (
                ...
                "Retake got worse grade — best still counts",
                "CSE",
                "1",
            ),
            (
                "tc53_retake_ineligible.csv",
                "Retake of B+ or above — NSU policy violation",
                "CSE",
                "1",
            ),
        ],
    ...
    (
        ...
        [
            (
                ...
                "BBA with FIN concentration (auto-detected)",
                "BBA",
                "3",
            ),
    ...
    (
        ...
        [
            ...
            (
                ...
                "Complete Physics Minor (4 req + prereqs)",
                "CSE",
                "3",
            ),
            (
                "tc56_cse_math_minor_partial.csv",
                "Partial Math Minor (1 of 3 electives)",
                "CSE",
                "3",
            ),
...
def cli_mode() -> None:
    ...
    tmp_csv_path = None
    if args.upload:
        console.print(f"[cyan]Running OCR on '{args.transcript}'...[/]")
        try:
            csv_str = extract_transcript_csv(args.transcript)
            with tempfile.NamedTemporaryFile("w", suffix=".csv", delete=False) as tmp:
                tmp.write(csv_str)
                tmp_csv_path = tmp.name
            args.transcript = tmp_csv_path
            console.print("[green]OCR complete! Proceeding with engine run.[/]")
        except Exception as e:
            console.print(f"[red]OCR Error:[/] {e}")
            sys.exit(1)
