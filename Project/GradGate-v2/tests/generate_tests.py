...
SEMESTERS = [f"{s} {y}" for y in range(2019, 2026) for s in ["Spring", "Summer", "Fall"]]
...
def main():
    ...
    out_dir = (
        Path(args.outdir) if args.outdir else Path(__file__).resolve().parent / "test_transcripts"
    )
        ...
        for i in range(n):
            ...
            with open(fname, "w", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["Course_Code", "Credits", "Grade", "Semester"]
                )
                writer.writeheader()
